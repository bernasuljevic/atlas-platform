...
public interface IWikiPageRepository
{
    ...
    // Klasör ağacı sorgusu (Gün 3) için - GetAllAsync TÜM departmanları
    // döndürüyor (cache'lenen "wiki-pages:all" için tasarlandı), bir tek
    // departmanın ağacını kurarken bu gereksiz veri çekimi olurdu.
    Task<IReadOnlyList<WikiPage>> GetByDepartmentAsync(string departmentName, CancellationToken ct = default);
